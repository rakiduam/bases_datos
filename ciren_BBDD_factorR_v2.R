...
#### DIRECTORIO ####
setwd('E:/Factor_R/BBDD/DGA_reporte_web_1980-2019/')
setwd('D:/Downloads/')
print(getwd())

# LECTURA DE DATOS ####
df <- as.data.frame(read_excel("BBDD PP 2.0.xlsx", col_names=TRUE, na="-9999"))
# df <- as.data.frame(read_excel("DGA-DMC_1980-2020.xlsx", col_names=TRUE, na="-9999"))
#df <- as.data.frame(read.csv("DGA-DMC_1980-2020.csv", sep = ';'))

old_df <- df

# LIMPIEZA DATAFRAME ####
df <- (df[c(15:nrow(df)),c(2:ncol(df))])
...
fecha <- (as.data.frame(str_split(c(as.character(seq.Date(from=as.Date('1987-01-31'),
...
#df[c(17:nrow(df)),1] <- cbind(fecha[, 1:2], df)
...
rownames(df) <- c(as.character(seq.Date(from = as.Date('1987-01-31'), to = as.Date('2020-01-01'), by = 'months')))
...
rango = c(as.character(seq.Date( from=as.Date('1987-01-31'), to=as.Date('2020-01-01'), by='months')))
# estimación #####
margen = 1
...
for (i in c(1:nrow(df))){
  if (df$n_data[i] >= 1){
    ...
    df$fecha_max[i] <- as.character( colnames(df)[ which( df[i, c(rango)] == df$max[i])] )
  } else{
  }
}

metadata <- (t(old_df[c(1:14),]))
...
write.table(cbind(metadata, df[c(3:nrow(df)),]),'D:/BBDD_estadistica.csv', sep=';')
